...
namespace KutuphaneOtomasyonu
{
    public partial class kitap_islemleri : Form
    {
        ...
        private void ekle_b_Click(object sender, EventArgs e)
        {
            ...
            int sonuc = k_islm.ktp_ekle(ad_t.Text, yazar_t.Text, no_t.Text, tur_t.Text,int.Parse(sayfasayısı_t.Text), yayınevi_t.Text);
            MessageBox.Show("Aramıza yeni bir kitap daha katıldı :D.");
        }
        ...
        private void guncelle_b_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                int Sonuc = ktp_guncelleme.ktp_guncelle(int.Parse(id_t.Text), ad_t.Text, yazar_t.Text, no_t.Text, tur_t.Text, int.Parse(sayfasayısı_t.Text), yayınevi_t.Text);
        ...
        private void kitap_listesi_d_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                sayfasayısı_t.Text = row.Cells[5].Value.ToString();
                yayınevi_t.Text= row.Cells[6].Value.ToString();
