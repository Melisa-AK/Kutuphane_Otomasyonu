    ...
    public class kitap_islem
    {
        ...
        public List<kitaplar> kitap_goster()
        {
            ...
            if (dr.HasRows)
            {
                ...
                while (dr.Read())
                {
                    kitaplar ktp = new kitaplar { kitapid = int.Parse(dr["kitapid"].ToString()), kitapadi = dr["kitapadi"].ToString(), yazar = dr["yazar"].ToString(), kitapnumara = dr["kitapnumara"].ToString(), tur = dr["tur"].ToString(), sayfasayısı= dr["sayfasayısı"].ToString(), yayinevi = dr["yayinevi"].ToString() };
                    kitaplar.Add(ktp);
                }
                return kitaplar;
            }
        ...
        public int ktp_ekle(string kitapadi, string yazar, string kitapnumara, string tur,int sayfasayısı, string yayinevi)
        {
            //kitap eklemek için sorgumuzu fonksiyona değerler ile birlikte gönderiyoruz.
            int sonuc = dl3.EkleSilGuncelle("insert into kitaplar (kitapadi,yazar,kitapnumara,tur,sayfasayısı,yayinevi) values ('" + kitapadi + "','" + yazar + "','" + kitapnumara + "','" + tur + "','" + sayfasayısı + "','" + yayinevi + "')", System.Data.CommandType.Text);
            return sonuc;
        }

        DAL.DAL dl4 = new DAL.DAL();
        public int ktp_guncelle(int kitapid, string kitapadi, string yazar, string kitapnumara, string tur, int sayfasayısı, string yayinevi)
        {
            //kitap güncellemek için sorgumuzu fonksiyona değerler ile birlikte gönderiyoruz.
            int sonuc = dl4.EkleSilGuncelle("update kitaplar set kitapadi='" + kitapadi + "', yazar='" + yazar + "', kitapnumara='" + kitapnumara + "', tur='" + tur + "','" + sayfasayısı + "','" + yayinevi + "' WHERE kitapid=" + kitapid + "", System.Data.CommandType.Text);
            return sonuc;
        }
