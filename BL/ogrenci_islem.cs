    ...
    public class ogrenci_islem
    {
        ...
        public List<ogrenciler> ogrenci_goster()
        {
            ...
            if (dr.HasRows)
            {
                ...
                while (dr.Read())
                {
                    ogrenciler ogrnci = new ogrenciler { ogrenciid = int.Parse(dr["ogrenciid"].ToString()), isim = dr["isim"].ToString(), numara = dr["numara"].ToString(), bolum = dr["bolum"].ToString(), email = dr["email"].ToString() , adres = dr["adres"].ToString() };
        ...
        public int ogr_ekle(string isim, string numara, string bolum, string email, string adres )
        {
            //öğrenci eklemek için sorgumuzu fonksiyona değerler ile birlikte gönderiyoruz.
            int sonuc = dl3.EkleSilGuncelle("INSERT into ogrenciler (isim,numara,bolum,email,adres) VALUES ('" + isim + "','" + numara + "','" + bolum + "','" + email + "','" + adres + "')", System.Data.CommandType.Text);
            return sonuc;
        }

        DAL.DAL dl4 = new DAL.DAL();
        public int ogr_guncelle(int ogrenciid, string isim, string numara, string bolum, string email, string adres)
        {
            //öğrenci güncellemek için sorgumuzu fonksiyona değerler ile birlikte gönderiyoruz.
            int sonuc = dl4.EkleSilGuncelle("UPDATE ogrenciler SET isim='" + isim + "', numara='" + numara + "', bolum='" + bolum + "', email='" + email + "', adres='" + adres + "' WHERE ogrenciid=" + ogrenciid + "", System.Data.CommandType.Text);
            return sonuc;
        }
